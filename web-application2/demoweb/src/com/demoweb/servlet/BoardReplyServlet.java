...
//		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/board/reply.jsp");
...
//		board.setContent(content);
		...
		for (FileItem item : items) {
			if (item.isFormField()) { //form-data인 경우
				...
				} else if (item.getFieldName().equals("boardno")) {
					String sBoardNo = item.getString("utf-8");
					if (sBoardNo == null) {
						board.setBoardNo(-1);
					} else {
						board.setBoardNo(Integer.parseInt(sBoardNo));
					}
				} else if (item.getFieldName().equals("pageno")) {
					sPageNo = item.getString("utf-8");
				}
		...
		if (board.getBoardNo() == -1 || sPageNo == null) {	//글번호, 페이지 번호 오류
			resp.sendRedirect("list.action");
			return;
		}
		...
		//4. 목록으로 이동
		resp.sendRedirect("list.action?pageno=" + sPageNo);
	}
